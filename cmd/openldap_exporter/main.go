...
func runMain(c *cli.Context) error {
	...
	/** Parse ldap address **/
	err := config.ParseAddr(c.String(ldapAddr))
	if err != nil {
	...
	if c.String(ldapCACrt) != "" {
		...
		if err != nil {
			...
			log.Println("Successfully loaded CA cert file:", c.String(ldapAddr))
		}
	}
	...
	if c.Bool(ldapUseStartTLS) {
		config.UseStartTLS = true
	}

	if c.Bool(ldapSkipInsecure) {
		config.SkipInsecure = true
	}
